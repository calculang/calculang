...
describe('bundle output (with a top-level console log call)', async () => {
  it('shop should match expected', async () => {
    const m = await compile({
      entrypoint: 'entry.cul.js',
      fs: {
...
console.log(sales({units_in: 10, sales_price_in:10})) // <-- TESTING THIS
`
      },
      memo: false
    });

    await expect(m.bundle).toMatchFileSnapshot('./bundles/shop-with-a-top-level-console-log-call.bundle.js');
  });
