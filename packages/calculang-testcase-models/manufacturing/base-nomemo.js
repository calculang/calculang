...
const revenue = ({ units_in, price_in }) => {
  return units({ units_in }) * price({ price_in });
};

// variable costs only, OK for testing
const costs = ({ units_in }) => 100 * units({ units_in });

const profit = ({ units_in, price_in }) => revenue({ units_in, price_in }) - costs({ units_in });

// inputs
const units = ({ units_in }) => units_in;
