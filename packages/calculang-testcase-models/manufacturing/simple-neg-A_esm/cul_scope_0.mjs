...
import { sales_by_product_ as sales_by_product$, product_ as product$, sales_total_ as sales_total$ } from "./cul_scope_1.mjs"; // there is already-culed stuff in here, why? imports to memo loader include cul_scope_id, what logic should it apply RE passing forward? eliminate? Probably!
...
export const sales_by_product = (a) => {
  ...
  sales_by_product$({ product_in }); // never run, but here to "trick" calculang graph logic
};
...
export const product = (a) => {
  return product$m(a);
  // eslint-disable-next-line no-undef
  product$({ product_in }); // never run, but here to "trick" calculang graph logic
};
...
export const sales_total = (a) => {
  ...
  // eslint-disable-next-line no-undef
  sales_total$({ product_in }); // never run, but here to "trick" calculang graph logic
};
